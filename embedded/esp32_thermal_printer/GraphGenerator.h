...
public:
  ...
  {
    const uint16_t numYDiv = yMax / yStep;
    const uint16_t valueIncrement = yStep * 10;

    int16_t xPos = graphStartX;
    uint16_t value = valueIncrement;

    char label[8];

    for (uint16_t i = 1; i <= numYDiv; ++i)
    {
      itoa(value, label, 10);

      canvas->drawText(
          label,
          xPos + gridYSpacing - 13,
          5,
          2,
          true);
